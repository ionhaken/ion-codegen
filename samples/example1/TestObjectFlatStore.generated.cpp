...
TestObjectFlatStore::TestObjectFlatStore()
{
	mBuffer = nullptr;
	mCapacity = 1000000;
	ResizeAndCopyBuffer(mCapacity, *this);
	SetName("TestObjectFlatStore");
}
...
void TestObjectFlatStore::ResizeBuffer(uint32_t capacity)
{
	ion::GlobalAllocator<uint8_t> allocator;
	auto* oldBuffer = mBuffer.Get();
	auto oldCapacity = mCapacity;
	uint32_t itemCount = mIndexPool.Size() - 1;
	uint32_t remapCount = mIndexPool.Max() - 1;
	...
	}
	{
		...
		mRemap.Replace(ion::AssumeAligned(reinterpret_cast<uint32_t *>(pos + (alignof(uint32_t) - offset))), remapCount);
	}
	{
		pos = pos + 0 + sizeof(uint32_t)*capacity + alignof(uint32_t);
		uintptr_t offset = reinterpret_cast<uintptr_t>(pos) % alignof(uint32_t);
		mReverseRemap.Replace(ion::AssumeAligned(reinterpret_cast<uint32_t *>(pos + (alignof(uint32_t) - offset))), itemCount);
	}
	mCapacity = capacity;
	allocator.deallocate(oldBuffer, BufferSize(oldCapacity));
}
void TestObjectFlatStore::ResizeAndCopyBuffer(uint32_t capacity, [[maybe_unused]] const TestObjectFlatStore& other)
{
	auto* oldBuffer = mBuffer.Get();
	auto oldCapacity = mCapacity;
	uint32_t itemCount = mIndexPool.Size();
	uint32_t otherItemCount = other.mIndexPool.Size();
	uint32_t remapCount = mIndexPool.Max();
	uint32_t otherRemapCount = other.mIndexPool.Max();
	ion::GlobalAllocator<uint8_t> allocator;
	mBuffer = allocator.allocate(BufferSize(capacity));
	uint8_t* pos = mBuffer.Get();
	{
	...
	}
	{
		pos = pos + 0 + sizeof(G3)*capacity + alignof(G3);
		uintptr_t offset = reinterpret_cast<uintptr_t>(pos) % alignof(uint32_t);
		mRemap.Reset(remapCount, ion::AssumeAligned(reinterpret_cast<uint32_t *>(pos + (alignof(uint32_t) - offset))));
		mRemap.CreateFrom(other.mRemap.Get(), otherRemapCount);
	}
	{
		pos = pos + 0 + sizeof(uint32_t)*capacity + alignof(uint32_t);
		uintptr_t offset = reinterpret_cast<uintptr_t>(pos) % alignof(uint32_t);
		mReverseRemap.Reset(itemCount, ion::AssumeAligned(reinterpret_cast<uint32_t *>(pos + (alignof(uint32_t) - offset))));
		mReverseRemap.CreateFrom(other.mReverseRemap.Get(), otherItemCount);
	}
	mCapacity = capacity;
	if (oldBuffer)
	{
...
void TestObjectFlatStore::CopyBuffer(const TestObjectFlatStore& other)
{
	uint32_t itemCount = mIndexPool.Size();
	uint32_t otherItemCount = other.mIndexPool.Size();
	uint32_t remapCount = mIndexPool.Max();
	...
	{
		mRemap.CopyFrom(other.mRemap.Get(), otherRemapCount, remapCount);
	}
	{
		mReverseRemap.CopyFrom(other.mReverseRemap.Get(), otherItemCount, itemCount);
	}
}
void TestObjectFlatStore::Clear()
{
	uint32_t itemCount = mIndexPool.Size();
	mG0.Clear(itemCount);
	mG1.Clear(itemCount);
	mG2.Clear(itemCount);
	mG3.Clear(itemCount);
	mRemap.Clear(mIndexPool.Max());
	mReverseRemap.Clear(itemCount);
}
