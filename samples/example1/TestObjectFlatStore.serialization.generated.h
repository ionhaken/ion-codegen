...
{
	bool Serialize(const ion::Vector<TestObjectFlatId>& ids, const TestObjectFlatStore& store, ion::JSONArrayWriter& data)
	{
		bool isOk = true;
		ion::ForEach(ids, [&](const auto& id)
		{
			...
			ConstTestObjectFlatComponent c(id, store);
	...
	bool Deserialize(ion::Vector<TestObjectFlatId>& ids, TestObjectFlatStore& store, const ion::JSONArrayReader& data)
	{
		if (!data.IsValid())
		{
			return false;
